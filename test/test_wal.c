...
int test_transaction_boundaries() {
  ...
  // First transaction
  if (db_put_table("txn1_key", "txn1_value") != 0)
    return 0;
  printf("  After first transaction: %llu\n",
         (unsigned long long)wal->next_txn_id);

  // Second transaction
  if (db_put_table("txn2_key", "txn2_value") != 0)
    return 0;
  ...
  int txn_ids_correct =
      (wal->next_txn_id == 3); // Should be 3 after 2 transactions
...
int test_wal_recovery() {
  ...
  if (db_put_table("recover_key1", "recover_value1") != 0)
    return 0;
  if (db_put_table("recover_key2", "recover_value2") != 0)
    return 0;
  if (db_delete_table("recover_key1") != 0)
    return 0; // Delete first key
  ...
  int recovery_correct =
      (val1 == NULL && val2 != NULL && strcmp(val2, "recover_value2") == 0);
...
int test_partial_write_detection() {
  ...
  // Add some valid data
  if (db_put_table("valid_key", "valid_value") != 0)
    return 0;
...
int main() {
  ...
  struct {
    const char *name;
    int (*test_func)();
  } tests[] = {{"Basic WAL Operations", test_basic_wal_operations},
